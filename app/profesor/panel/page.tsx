...
//  SOCKET EFECTO REAL
useEffect(() => {
  const s = io("https://http://localhost:3001")
  setSocket(s)
  ...
  const abrirIntegrantes = async (salon: Salon) => {
    ...
    try {
      const res = await fetch(`https://http://localhost:3001/api/alumnos_temporales?codigo=${encodeURIComponent(salon.codigo.trim())}`)
