...
//  SOCKET EFECTO REAL
useEffect(() => {
  const s = io("http://34.130.57.30/api/login")
  setSocket(s)
  ...
  const abrirIntegrantes = async (salon: Salon) => {
    ...
    try {
      const res = await fetch(`http://34.130.57.30/api/login/api/alumnos_temporales?codigo=${encodeURIComponent(salon.codigo.trim())}`)
