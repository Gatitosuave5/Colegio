...
useEffect(() => {
  const cargarAlumno = async () => {
    const res = await fetch(`http://34.130.10.63:3001/api/alumno?nombre=${nombreAlumno}&salon=${codigo}`);
...
useEffect(() => {
  const cargarContenidos = async () => {
    try {
      const res = await fetch(`http://34.130.10.63:3001/api/contenidos?codigo=${codigo}`);
      const data = await res.json();
...
useEffect(() => {
  ...
  const handler = () => {
    ...
    fetch("http://34.130.10.63:3001/api/alumnos_temporales/eliminar", {
      method: "POST",
  ...
  const API = "http://34.130.10.63:3001";
    ...
    <div className="flex items-center gap-6">
      ...
      >
        ...
        Tabla
            ...
            <button
        onClick={async () => {
          await fetch("http://34.130.10.63:3001/api/alumnos_temporales/eliminar", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              nombre: nombreAlumno,
