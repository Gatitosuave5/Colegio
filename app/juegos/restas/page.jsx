...
export default function RestasPage() {
  ...
  const [num2, setNum2] = useState(Math.floor(Math.random() * 5));
  ...
  const verificar = () => {
    if (parseInt(respuesta) === num1 - num2) {
      ...
      setMensaje("❌ Revisa tu respuesta");
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#fff5f8] p-6">
      <button
        onClick={() => router.back()}
        className="absolute top-5 left-5 flex items-center text-pink-600 hover:underline"
      >
