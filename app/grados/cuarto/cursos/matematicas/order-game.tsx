...
export default function OrderGame({ topic, onBack }) {
  ...
  const generateRound = () => {
    ...
    else if (topic.id === "suma-resta-grandes") {
      const operations = [
        { val: 358023, display: "234,567 + 123,456" },
        { val: 554889, display: "789,456 - 234,567" },
        { val: 750000, display: "500,000 + 250,000" },
        { val: 888888, display: "999,999 - 111,111" },
        { val: 580245, display: "123,456 + 456,789" },
      ];
      newItems = operations.map((op, idx) => ({
        id: `op-${idx}`,
        value: op.val,
    ...
    else if (topic.id === "division-exacta-resto") {
      const divisions = [
        { val: 0, display: "25 ÷ 4 (resto)" },
        { val: 0, display: "36 ÷ 9 (exacta)" },
        { val: 1, display: "50 ÷ 7 (resto)" },
        { val: 9, display: "45 ÷ 5 (cociente)" },
        { val: 3, display: "33 ÷ 5 (resto)" },
      ];
      newItems = divisions.map((div, idx) => ({
        id: `div-${idx}`,
        value: div.val,
        displayValue: div.display,
      }));
    }
    else if (topic.id === "comparacion-fracciones") {
      const fractions = [
        { val: 1, display: "1/4 o 1/2" },
        { val: 2, display: "2/3 o 2/5" },
        { val: 3, display: "3/4 o 3/5" },
        { val: 4, display: "5/6 o 5/8" },
        { val: 5, display: "1/3 o 1/4" },
      ];
      newItems = fractions.map((frac, idx) => ({
        id: `frac-${idx}`,
        value: frac.val,
        displayValue: frac.display,
      }));
    }
