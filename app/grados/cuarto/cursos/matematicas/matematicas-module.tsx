...
import { topicsData4 } from "./topicsData";
import TopicReader from "./topic-reader";
import TopicQuiz from "./topic-quiz";
import TopicGames from ".";

export const mathStoryMapGrade4: Record<string, string> = {
  "sumas-4to": "sumas-4to",
  "restas-4to": "restas-4to",
  "multiplicacion-4to": "multiplicacion-4to",
  "division-4to": "division-4to",
  "figuras-4to": "figuras-4to",
  "clasificacion-4to": "clasificacion-4to",
};
...
export default function MatematicasModule4({ onBack, contenidosActivos }: MatematicasModuleProps) {
  const [currentView, setCurrentView] = useState<View>("list");
  const [selectedTopic, setSelectedTopic] = useState<any | null>(null);

  const topics = contenidosActivos
  .map(c => mathStoryMapGrade4[c.storyId])   // convierte storyId a topicId
  .filter(Boolean)                           // elimina null / undefined
  .map(topicId => topicsData4[topicId]); 
