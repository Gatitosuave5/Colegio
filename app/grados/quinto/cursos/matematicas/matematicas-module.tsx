...
import { topicsData5 } from "./topicsData";
import TopicReader from "./topic-reader";
import TopicQuiz from "./topic-quiz";
import TopicGames from ".";

export const mathStoryMapGrade5: Record<string, string> = {
  "sumas-restas-5to": "sumas-restas-5to",
  "multiplicacion-5to": "multiplicacion-5to",
  "division-5to": "division-5to",
  "fracciones-5to": "fracciones-5to",
  "decimales-5to": "decimales-5to",
  "area-5to": "area-5to",
  "graficos-5to": "graficos-5to",
  "patrones-5to": "patrones-5to",
  "problemas-5to": "problemas-5to",
  "ordenar-5to": "ordenar-5to",
};
...
export default function MatematicasModule5({ onBack, contenidosActivos }: MatematicasModuleProps) {
  const [currentView, setCurrentView] = useState<View>("list");
  const [selectedTopic, setSelectedTopic] = useState<any | null>(null);

  const topics = contenidosActivos
  .map(c => mathStoryMapGrade5[c.storyId])   // convierte storyId a topicId
  .filter(Boolean)                           // elimina null / undefined
  .map(topicId => topicsData5[topicId]); 
