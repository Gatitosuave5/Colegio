...
export default function MemoryGame({ topic, onBack }) {
  ...
  // Inicializar juego según el tema
  useEffect(() => {
    initializeGame();
  }, [topic]);
  ...
  const initializeGame = () => {
    ...
    if (topic.id === "sumas-1-10") {
      ...
      const pairs = [
        ...
        { shape: "✡️ Estrella", desc: "Puntiaguda" },
      ];
      gameCards = pairs.flatMap((pair, idx) => [
        {
          id: `shape-${idx}`,
    ...
    } else if (topic.id === "conteo") {
      const pairs = [
        { item: "🍎 1 manzana", count: "1" },
        { item: "🎈 2 globos", count: "2" },
        { item: "⭐ 3 estrellas", count: "3" },
        { item: "🎁 4 regalos", count: "4" },
        { item: "🌸 5 flores", count: "5" },
        { item: "🐝 6 abejas", count: "6" },
      ];
      gameCards = pairs.flatMap((pair, idx) => [
        {
          id: `item-${idx}`,
