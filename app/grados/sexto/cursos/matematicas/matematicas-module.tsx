...
import { topicsData6 } from "./topicsData";
import TopicReader from "./topic-reader";
import TopicQuiz from "./topic-quiz";
import TopicGames from ".";

export const mathStoryMapGrade6: Record<string, string> = {
  "operaciones-6to": "operaciones-6to",
  "multiplos-6to": "multiplos-6to",
  "fracciones-6to": "fracciones-6to",
  "porcentajes-6to": "porcentajes-6to",
  "area-6to": "area-6to",
  "volumen-6to": "volumen-6to",
  "coordenadas-6to": "coordenadas-6to",
  "estadistica-6to": "estadistica-6to",
  "patrones-6to": "patrones-6to",
  "razonamiento-6to": "razonamiento-6to"
};
...
export default function MatematicasModule6({ onBack, contenidosActivos }: MatematicasModuleProps) {
  const [currentView, setCurrentView] = useState<View>("list");
  const [selectedTopic, setSelectedTopic] = useState<any | null>(null);

  const topics = contenidosActivos
  .map(c => mathStoryMapGrade6[c.storyId])   // convierte storyId a topicId
  .filter(Boolean)                           // elimina null / undefined
  .map(topicId => topicsData6[topicId]); 
